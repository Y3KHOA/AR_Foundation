...
public class BtnController : MonoBehaviour
{
    ...
    private void CreateCompassLabel(float heading)
    {
        ...
        if (nearestWall != null)
        {
            // Hướng từ vị trí spawn đến điểm gần nhất trên tường
            ...
            euler.x = 90f; // ép cứng X
            euler.z = 90f; // ép cứng Z
            ...
            // Gán hướng compass vào TextMesh nếu muốn
            TextMesh tm = label.GetComponentInChildren<TextMesh>();
            if (tm != null)
                tm.text = $"{heading:0}° {GetCompassDirectionName(heading)}";

            Debug.Log($"Compass Label được tạo tại vị trí: {spawnPosition}, hướng về tường gần nhất.");
        }
        else
        {
