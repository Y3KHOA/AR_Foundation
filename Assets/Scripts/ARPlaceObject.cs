...
public class ARMeasureTool : MonoBehaviour
{
    ...
    void PlaceCheckpoint(Vector3 newPoint)
    {
        ...
        if (points.Count > 1)
        {
            lineManager.DrawLineAndDistance(points[^2], newPoint, lineManager.distanceTextPrefab);
        }
    }

    void CloseMeasurement()
    {
        lineManager.DrawLineAndDistance(points[^1], points[0], lineManager.distanceTextPrefab);
